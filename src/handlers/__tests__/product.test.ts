...
describe('POST /api/products', () => {
  ...
  it("should valdiate that the price is greater than zero", async () => {
    const response = await request(server).post("/api/products").send({
      name: "Curved Monitor",
      price: -300,
    });
  ...
  it("should valdiate that the price is a number and greater than zero", async () => {
    const response = await request(server).post("/api/products").send({
      name: "Curved Monitor",
      price: "zero",
    });
...
describe('PUT /api/products/:id', () => {
  ...
  it("should validate that the price is greater than 0", async () => {
    ...
    expect(response.body.errors).toBeTruthy();
    expect(response.body.errors).toHaveLength(1);
    expect(response.body.errors[0].msg).toBe("TThe value is not valid");
  ...
  it("should update an existing product with valid data", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.error).toBe("data");
...
describe('DELETE /api/products/:id', () => {
  ...
  it('should delete a product', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.data).toBe('The product was eliminated');
